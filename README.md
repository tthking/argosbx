...
#### 3、支持各种容器系统，Docker / Node.js 镜像部署（公开镜像库：`imccn/argosbx`）。**🚀【最新增强】Node.js 容器版原生集成了 Web 控制台**（访问路径 `/<您设置的uuid>`），支持直接在浏览器页面无代码一键生成、查阅通用订阅、及**一键热重载更新本机配置**！

#### 4、根据Sing-box与Xray不同内核，可选15种WARP出站组合，更换落地IP为WARP的IP，解锁流媒体。**🚀【最新增强】新增环境 WARP 碰撞智能探测逻辑**：安装时支持交互式强制叠加“双重套娃WARP”以满足全能需求；并提供了完美的卸载进程，同步无痕扫除内置核心的 WARP 参数挂载！
...
#### 6、建议配合可视化的一键命令生成器网页使用：https://tthking.github.io/argosbx/ \n**(💡 注：如果您运行的是 Node 版应用，启动后直接访问根目录的 `/<您的uuid>` 路径即可在 Web 端畅享生成、订阅乃至本机直接部署的丝滑一体化体验，彻底告别 SSH 复制粘贴！)**

#### 7、如需要多样的功能，推荐使用VPS专用四合一脚本[Sing-box-yg](https://github.com/tthking/sing-box-yg)
...
* 默认主脚本curl：```bash <(curl -Ls https://raw.githubusercontent.com/tthking/argosbx/main/argosbx.sh)```

* 如报错curl not found 可换用主脚本wget：```bash <(wget -qO- https://raw.githubusercontent.com/tthking/argosbx/main/argosbx.sh)```
...
* ### 模版1：多个任意协议组合运行
```
sspt="" vlpt="" vmpt="" vwpt="" hypt="" tupt="" xhpt="" vxpt="" anpt="" arpt="" sopt="" bash <(curl -Ls https://raw.githubusercontent.com/tthking/argosbx/main/argosbx.sh)
```
...
```
vlpt="" bash <(curl -Ls https://raw.githubusercontent.com/tthking/argosbx/main/argosbx.sh)
```

Vless-Xhttp-Reality-vision-enc协议节点 (默认开启ENC加密)
```
xhpt="" bash <(curl -Ls https://raw.githubusercontent.com/tthking/argosbx/main/argosbx.sh)
```

Vless-Xhttp-vision-enc协议节点 (默认开启ENC加密，IDX-Google-VPS容器支持)
```
vxpt="" bash <(curl -Ls https://raw.githubusercontent.com/tthking/argosbx/main/argosbx.sh)
```

Vless-ws-vision-enc协议节点 (默认开启ENC加密)
```
vwpt="" bash <(curl -Ls https://raw.githubusercontent.com/tthking/argosbx/main/argosbx.sh)
```

Shadowsocks-2022协议节点
```
sspt="" bash <(curl -Ls https://raw.githubusercontent.com/tthking/argosbx/main/argosbx.sh)
```

AnyTLS协议节点
```
anpt="" bash <(curl -Ls https://raw.githubusercontent.com/tthking/argosbx/main/argosbx.sh)
```

Any-Reality协议节点
```
arpt="" bash <(curl -Ls https://raw.githubusercontent.com/tthking/argosbx/main/argosbx.sh)
```

Vmess-ws协议节点
```
vmpt="" bash <(curl -Ls https://raw.githubusercontent.com/tthking/argosbx/main/argosbx.sh)
```

Socks5协议节点 (配合其他应用内置代理使用，勿做节点直接使用)
```
sopt="" bash <(curl -Ls https://raw.githubusercontent.com/tthking/argosbx/main/argosbx.sh)
```

Hysteria2协议节点
```
hypt="" bash <(curl -Ls https://raw.githubusercontent.com/tthking/argosbx/main/argosbx.sh)
```

Tuic协议节点
```
tupt="" bash <(curl -Ls https://raw.githubusercontent.com/tthking/argosbx/main/argosbx.sh)
```
...
```
vmpt="" argo="vmpt" bash <(curl -Ls https://raw.githubusercontent.com/tthking/argosbx/main/argosbx.sh)
```

Vless-ws-vision-enc-argo临时隧道CDN优选节点
```
vwpt="" argo="vwpt" bash <(curl -Ls https://raw.githubusercontent.com/tthking/argosbx/main/argosbx.sh)
```

Vmess-ws-argo-argo固定隧道CDN优选节点，必须填写端口(vmpt)、域名(agn)、token(agk)
```
vmpt="CF设置的URL端口" argo="vmpt" agn="解析的CF域名" agk="CF获取的token" bash <(curl -Ls https://raw.githubusercontent.com/tthking/argosbx/main/argosbx.sh)
```

Vless-ws-vision-enc-argo固定隧道CDN优选节点，必须填写端口(vmpt)、域名(agn)、token(agk)
```
vwpt="CF设置的URL端口" argo="vwpt" agn="解析的CF域名" agk="CF获取的token" bash <(curl -Ls https://raw.githubusercontent.com/tthking/argosbx/main/argosbx.sh)
```

Vmess-ws的80系端口、回源端口的CDN优选节点
```
vmpt="80系端口、指定回源端口" cdnym="CF解析IP的域名" bash <(curl -Ls https://raw.githubusercontent.com/tthking/argosbx/main/argosbx.sh)
```

Vless-Xhttp-vision-enc的80系端口、回源端口的CDN优选节点
```
vxpt="80系端口、指定回源端口" cdnym="CF解析IP的域名" bash <(curl -Ls https://raw.githubusercontent.com/tthking/argosbx/main/argosbx.sh)
```

Vless-ws-vision-enc的80系端口、回源端口的CDN优选节点
```
vwpt="80系端口、指定回源端口" cdnym="CF解析IP的域名" bash <(curl -Ls https://raw.githubusercontent.com/tthking/argosbx/main/argosbx.sh)
```
...
----------------------------------------------------------
### 感谢你右上角的star🌟
[![Stargazers over time](https://starchart.cc/tthking/ArgoSB.svg)](https://starchart.cc/tthking/ArgoSB)
